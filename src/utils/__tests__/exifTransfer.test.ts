...
describe("normalizeExifForBakedImage", () => {
  ...
  describe("stripThumbnail オプション（レタッチ経路の未編集画像リーク防止）", () => {
    ...
    it("IFD1（EXIF サムネイル）を除去し、他のタグは保持する", async () => {
    ...
    it("stripThumbnail なしではサムネイルが保持される（既存経路の互換）", async () => {
      const tiff = buildTiffWithThumbnail();
      const normalized = await normalizeExifForBakedImage(tiff, 100, 200);
      expect(loadExif(normalized).thumbnail).toBeTruthy();
    });

    it("サムネイルが無い TIFF でも例外を投げず正常に処理する", async () => {
      const tiff = piexifDumpToTiff(
        piexif.dump({
    ...
    it("正規化に失敗した場合、stripThumbnail 指定時は元 TIFF を返さず例外を投げる", async () => {
      // パース不能な壊れた TIFF。通常経路は「EXIF 保持を優先」して元 TIFF を返すが、
      // stripThumbnail 指定時は未編集サムネイルが残り得るため失敗させる
      const broken = new Uint8Array([1, 2, 3, 4]);
      await expect(normalizeExifForBakedImage(broken, 100, 200)).resolves.toBe(
        broken,
      );
      await expect(
