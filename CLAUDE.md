...
- `src/utils/` - コアユーティリティクラス
  ...
  - `webglImageRenderer.ts` - ライト/カラー調整の描画オーケストレーション（`isWebGLPipelineSupported`（`isOffscreenPipelineSupported` と同思想）/ 永続 WebGL2 レンダラ `createAdjustmentRenderer`（テクスチャ・program 使い回し・dispose）/ プレビューと出力の共通経路 `applyAdjustmentsToCanvas`（renderer 有=GPU・null=Canvas2D CPU フォールバック））。Canvas/WebGL 依存のため E2E 検証
...
- 画像編集（ライト/カラー調整）はプレビュー・出力とも同一の `applyAdjustmentsToCanvas`（`webglImageRenderer.ts`）を通す（WYSIWYG）。既定は WebGL2 フラグメントシェーダ（`adjustmentShader.ts`）でリアルタイム描画し、`isWebGLPipelineSupported()` が false の環境は Canvas2D + `applyAdjustmentToPixel`（`adjustments.ts`）の CPU パスへフォールバックする。GPU（GLSL）と CPU（TS）が結果一致する構造的担保として、ピクセル単位の色調整数式は `adjustments.ts` の `applyAdjustmentToPixel` を唯一の真実とし、GLSL 側は同じ順序・同じ係数・同じクランプ位置でミラーする（キー→uniform 名の対応表 `ADJUSTMENT_UNIFORMS` も単一の真実として配線・アップロード・単体テストのガードが共有）。バッチ（`editImages`）は WebGL レンダラを 1 個だけ生成して全ファイルで使い回し、finally で必ず dispose する（コンテキスト枯渇を防ぐ。crop 同様メインスレッド逐次処理で、Worker + OffscreenCanvas 化は将来の最適化）。調整は非破壊で `AdjustmentState`（UI 単位 [-100,100]）として保持し、書き出し時に `normalizeAdjustments` で [-1,1] に正規化してから適用する。適用範囲（全画像一括 / 画像ごと）は crop の dual-store パターンを踏襲する（`resolveAdjustmentForIndex`）。UI は `AdjustmentPanel`（`ConversionSettings` の不変更新契約を踏襲）/ `EditToolbar` / `CompareView`（文言は i18n `edit.*`。適用範囲・画像ナビは `crop.*` を共用、失敗通知の見出しは `ConversionErrors` の `titleKey` prop で `edit.editFailures` に差し替え）
