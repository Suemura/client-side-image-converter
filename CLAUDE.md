...
12. **統合ワークスペース「Image Studio」**（`/studio`）- 6 ツール（トリミング・編集・レタッチ・AI拡大・背景除去・メタデータ）を同一画面で操作する Photoshop ライク。線形パイプライン + 20 コミット undo/redo + 履歴パネル（`editHistory.ts`）。組み込み export（リネーム規則対応）。レタッチは顔・ナンバーの AI 自動検出→領域変換可
