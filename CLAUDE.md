...
- `src/app/` - Next.js App Router ページ
  ...
  - `edit/` - 画像編集ページ（写真アプリ相当のライト/カラー調整、Issue #66。`components/AdjustmentPanel.tsx`（ライト/カラーのスライダー群）・`components/EditToolbar.tsx`（適用範囲・出力形式・EXIF 保持・全体リセット）・`components/CompareView.tsx`（分割 Before/After 比較・画像ナビ・WebGL レンダラ保持）で構成）
...
- `src/utils/` - コアユーティリティクラス
  ...
  - `adjustmentShader.ts` - WebGL2 シェーダ文字列生成の Canvas/WebGL 非依存な純粋ロジック（`buildAdjustmentShader`（GLSL ES 3.00。`applyAdjustmentToPixel` を同順序・同係数でミラー）/ `VERTEX_SHADER_SOURCE` / キー→uniform 名の対応表 `ADJUSTMENT_UNIFORMS`（配線の単一の真実））。単体テスト対象
  - `webglImageRenderer.ts` - ライト/カラー調整の描画オーケストレーション（永続 WebGL2 レンダラ `createAdjustmentRenderer`（WebGL2 非対応・コンテキスト取得失敗・シェーダコンパイル失敗のいずれでも null を返すため呼び出し側の事前可用性チェックは不要。テクスチャ・program 使い回し、同一ソースでは調整値のみ変更時にテクスチャ再アップロードを省略、dispose）/ プレビューと出力の共通経路 `applyAdjustmentsToCanvas`（renderer 有=GPU・null=Canvas2D CPU フォールバック））。Canvas/WebGL 依存のため E2E 検証
  - `imageEditor.ts` - 画像編集のブラウザ側オーケストレーション（型 `EditJob` / `EditOutputFormat` / `EditOptions` / `BatchEditResult`、`renderOrientedImage`→調整適用→エンコード→EXIF 挿入の `renderEdited`、レンダラを 1 個生成して全ファイルで使い回し finally dispose する `editImages`（失敗一覧を返す））。Canvas/WebGL 依存のため E2E 検証
...
3. **画像編集（ライト/カラー調整）** - 写真アプリ相当のライト/カラー調整を非破壊で行い複数画像へまとめて適用できる `/edit` ページ（Phase 1 MVP、Issue #66）。ライト（露光量 / 輝度 / コントラスト / ハイライト / シャドウ / 白レベル / 黒レベル）とカラー（彩度 / 自然な彩度（Vibrance）/ 色温度 / 色合い（Tint）/ 色相）を各項目リセット + 全体リセット付きで調整する。プレビューは WebGL2 フラグメントシェーダでリアルタイム反映し、非対応環境は Canvas2D CPU パスへ自動フォールバック（プレビューと出力は同一の描画経路 `applyAdjustmentsToCanvas` を通すため結果が一致＝WYSIWYG）。分割スライダーの Before/After 比較、適用範囲トグル（全画像一括 / 画像ごと。crop と同じ dual-store で ←/→ と連動）に対応。出力は元形式維持（既定）または形式選択（JPEG/PNG/WebP/AVIF）で、EXIF 保持は AVIF 以外で有効（向き焼き込みに合わせ Orientation を 1 に正規化）。受理形式は crop と同じ UPLOAD_FORMATS（JPEG/PNG/WebP/BMP。HEIC/TIFF はプレビュー描画不可のため対象外）。当面はメインスレッド逐次処理（crop 同様）
...
- 画像編集（ライト/カラー調整）はプレビュー・出力とも同一の `applyAdjustmentsToCanvas`（`webglImageRenderer.ts`）を通す（WYSIWYG）。既定は WebGL2 フラグメントシェーダ（`adjustmentShader.ts`）でリアルタイム描画し、`createAdjustmentRenderer` が null を返す環境（WebGL2 非対応）は Canvas2D + `applyAdjustmentToPixel`（`adjustments.ts`）の CPU パスへフォールバックする。GPU（GLSL）と CPU（TS）が結果一致する構造的担保として、ピクセル単位の色調整数式は `adjustments.ts` の `applyAdjustmentToPixel` を唯一の真実とし、GLSL 側は同じ順序・同じ係数・同じクランプ位置でミラーする（キー→uniform 名の対応表 `ADJUSTMENT_UNIFORMS` も単一の真実として配線・アップロード・単体テストのガードが共有）。バッチ（`editImages`）は WebGL レンダラを 1 個だけ生成して全ファイルで使い回し、finally で必ず dispose する（コンテキスト枯渇を防ぐ。crop 同様メインスレッド逐次処理で、Worker + OffscreenCanvas 化は将来の最適化）。調整は非破壊で `AdjustmentState`（UI 単位 [-100,100]）として保持し、書き出し時に `normalizeAdjustments` で [-1,1] に正規化してから適用する。適用範囲（全画像一括 / 画像ごと）は crop の dual-store パターンを踏襲する（`resolveAdjustmentForIndex`）。UI は `AdjustmentPanel`（`ConversionSettings` の不変更新契約を踏襲）/ `EditToolbar` / `CompareView`（文言は i18n `edit.*`。適用範囲・画像ナビは `crop.*` を共用、失敗通知の見出しは `ConversionErrors` の `titleKey` prop で `edit.editFailures` に差し替え）
...
- Canvas API や WASM / WebGL に依存する処理（描画・変換・エンコード）は happy-dom では動作しないため、単体テストの対象外とする（純粋ロジック部分を切り出してテストする）。編集ページも同様で、色調整の数式（`adjustments.ts` の `applyAdjustmentToPixel` 等）とシェーダ生成・配線（`adjustmentShader.ts`）を純粋ロジックとして単体テスト（`adjustments.test.ts` / `adjustmentShader.test.ts`）し、WebGL/Canvas を使う実描画は E2E（`e2e/edit.spec.ts`）で検証する
