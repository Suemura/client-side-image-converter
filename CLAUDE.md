...
## 最近の更新
- 画像編集ページ `/edit` にトーンカーブ（RGB / 輝度チャンネル）を追加（Issue #68 の第 2 項目。他項目は未実装）。右カラムの調整スライダーと LUT ピッカーの間（パイプライン順）に SVG カーブエディタを配置し、プロット領域のクリックで制御点を追加してそのままドラッグ配置・ドラッグで移動・ダブルクリックで削除できる（端点は x 固定で y のみ可動・削除不可、最大 16 点・隣接最小間隔 0.01。背景に輝度ヒストグラム（PR #74 の `buildHistogramPath` を再利用）を重畳、チャンネル単位の「カーブをリセット」付き。全体リセット・適用範囲トグル・`hasAdjustments` 判定にも統合）。適用順は 調整 → トーンカーブ → LUT に固定し、プレビューと出力は同一経路（WYSIWYG）・WebGL2 非対応環境は CPU フォールバック。全処理クライアントサイド完結・依存追加なし。純粋ロジック `toneCurve.ts`（型 `CurvePoint` / `CurveChannel` / `ToneCurveState` / `ToneCurveEditState`、点操作 `addCurvePoint` / `moveCurvePoint` / `removeCurvePoint`（拒否時は同一配列参照を返す契約）、Fritsch–Carlson 単調 3 次 Hermite 補間 `evaluateCurve` / `buildCurveLut`、GPU/CPU 共通テーブル焼成 `buildToneCurveTable`（RGBA インターリーブ Float32Array(256×4)。`.rgb`=各チャンネル（現状マスター同値・将来 R/G/B 個別へ拡張可能）/ `.a`=輝度）、GPU の LINEAR サンプリングと同式の `sampleCurveTable`（floor+lerp）、CPU 適用 `applyToneCurveToPixel`（RGB マスター → 輝度カーブの加算シフト。Rec.709 luma）、`isDefaultToneCurve` / dual-store 解決 `resolveToneCurveForIndex`）と UI `edit/components/ToneCurvePanel.tsx`（数式は toneCurve.ts へ委譲し座標変換とイベント処理に徹する）を新設。`adjustmentShader.ts` に `CURVE_SAMPLER` / `CURVE_UNIFORMS`（配線の単一の真実）とカーブブロック（調整クランプ後・LUT 前。テクセル中心補正 (v*255+0.5)/256 で CPU と一致）、`webglImageRenderer.ts` に 256×1 RGBA8 テクスチャ（TEXTURE2・既定恒等・参照比較 `lastCurveTable` で再アップロード省略）と `render` / `applyAdjustmentsToCanvas` の末尾オプション引数 `curve`、`imageEditor.ts` の `EditJob` に `curve` を追加。dual-store（一括 / 画像ごと）は調整・LUT と同じ `applyToAll` トグルを共有し、恒等時は null（サンプリングスキップ）・出力時は同一 `ToneCurveState` のテーブルを Map で重複焼成しない。i18n `edit.toneCurve.*`（日英）を追加。単体テストは `toneCurve.test.ts`（24 件。`adjustmentShader.test.ts` に配線ガード + 適用順（カーブが LUT より前）の構造ガードを追加）、実ブラウザ検証は `e2e/edit.spec.ts`（点追加でプレビュー・出力が明るくなる WYSIWYG とリセット復帰・輝度チャンネルの色味（R>G>B の序列）維持・WebGL2 無効化の CPU フォールバック）。convert / crop / metadata と編集の既存仕様（調整・LUT・ヒストグラム・出力形式・EXIF 保持）は不変
