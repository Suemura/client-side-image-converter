...
- `src/app/` - Next.js App Router ページ
  ...
  - `edit/` - 画像編集ページ（写真アプリ相当のライト/カラー調整 + LUT フィルタ + ヒストグラム表示、Issue #66 / #67 / #68 の第 1 項目。`components/AdjustmentPanel.tsx`（ライト/カラーのスライダー群）・`components/EditToolbar.tsx`（適用範囲・出力形式・EXIF 保持・全体リセット）・`components/CompareView.tsx`（分割 Before/After 比較・画像ナビ・WebGL レンダラ保持。編集後プレビューの描画完了ごとに縮小サンプリングした ImageData を渡す optional prop `onEditedFrame` 付き）・`components/LutPicker.tsx`（LUT プリセットサムネイル一覧・「なし」・カスタムアップロード（.cube / HALD PNG）・適用強度スライダー・エラー通知）・`components/HistogramPanel.tsx`（調整・LUT 適用後のヒストグラムを SVG パスで表示。RGB / 輝度の切替チップ）で構成）
...
- `src/utils/` - コアユーティリティクラス
  ...
  - `adjustmentShader.ts` - WebGL2 シェーダ文字列生成の Canvas/WebGL 非依存な純粋ロジック（`buildAdjustmentShader`（GLSL ES 3.00。`applyAdjustmentToPixel` を同順序・同係数でミラー。調整の後段に `precision highp sampler3D` を使う LUT 色変換フィルタ（ドメイン正規化 → 3D テクスチャのトライリニア lookup → 強度ブレンド）を実装し、`applyLutToPixel` を同順序でミラー）/ `VERTEX_SHADER_SOURCE` / キー→uniform 名の対応表 `ADJUSTMENT_UNIFORMS`（調整の配線の単一の真実）/ LUT サンプラー名 `LUT_SAMPLER` と LUT uniform 名の対応表 `LUT_UNIFORMS`（LUT の配線の単一の真実））。単体テスト対象
  ...
  - `webglImageRenderer.ts` - ライト/カラー調整（+ LUT）の描画オーケストレーション（永続 WebGL2 レンダラ `createAdjustmentRenderer`（WebGL2 非対応・コンテキスト取得失敗・シェーダコンパイル失敗のいずれでも null を返すため呼び出し側の事前可用性チェックは不要。2D 画像テクスチャ・LUT 3D テクスチャ（TEXTURE1・内部形式 RGB8。既定は恒等 LUT で常時 complete）・program 使い回し、同一ソースでは調整値のみ変更時にテクスチャ再アップロードを省略・LUT もデータ参照が変わったときだけ再アップロード、dispose）/ プレビューと出力の共通経路 `applyAdjustmentsToCanvas`（renderer 有=GPU・null=Canvas2D CPU フォールバックで `applyLutToPixel` を調整の後段に適用）/ 型 `LutApplication`（LUT データ + 強度））。Canvas/WebGL 依存のため E2E 検証
  - `imageEditor.ts` - 画像編集のブラウザ側オーケストレーション（型 `EditJob`（`lut?: LutApplication | null` を含む） / `EditOutputFormat` / `EditOptions` / `BatchEditResult`、`renderOrientedImage`→調整・LUT 適用→エンコード→EXIF 挿入の `renderEdited`、レンダラを 1 個生成して全ファイルで使い回し finally dispose する `editImages`（失敗一覧を返す））。Canvas/WebGL 依存のため E2E 検証
...
3. **画像編集（ライト/カラー調整 + LUT フィルタ）** - 写真アプリ相当のライト/カラー調整と LUT（Look-Up Table）フィルタを非破壊で行い複数画像へまとめて適用できる `/edit` ページ（Phase 1 MVP = Issue #66、Phase 2 LUT = Issue #67）。ライト（露光量 / 輝度 / コントラスト / ハイライト / シャドウ / 白レベル / 黒レベル）とカラー（彩度 / 自然な彩度（Vibrance）/ 色温度 / 色合い（Tint）/ 色相）を各項目リセット + 全体リセット付きで調整する。LUT フィルタは同梱プリセット（11 本）から選ぶか、`.cube`（1D / 3D）または HALD CLUT PNG をカスタムアップロードして適用でき、適用強度（0-100）で元色とのブレンド比を調整できる（LUT は調整の後段に適用）。プレビューは WebGL2 フラグメントシェーダでリアルタイム反映し、非対応環境は Canvas2D CPU パスへ自動フォールバック（プレビューと出力は同一の描画経路 `applyAdjustmentsToCanvas` を通すため結果が一致＝WYSIWYG）。分割スライダーの Before/After 比較、適用範囲トグル（全画像一括 / 画像ごと。crop と同じ dual-store で ←/→ と連動。調整と LUT は同じトグルを共有）に対応。右カラム先頭（調整スライダーの上）には調整・LUT 適用**後**のプレビュー結果を反映するヒストグラム（RGB / 輝度の表示切替。黒/白レベル調整の指標、Issue #68 の第 1 項目）を常時表示し、スライダー操作にリアルタイム追従する（rAF で 1 フレーム 1 回に間引き。WebGL2 非対応の CPU フォールバック環境でも同一経路で表示）。出力は元形式維持（既定）または形式選択（JPEG/PNG/WebP/AVIF）で、EXIF 保持は AVIF 以外で有効（向き焼き込みに合わせ Orientation を 1 に正規化）。受理形式は crop と同じ UPLOAD_FORMATS（JPEG/PNG/WebP/BMP。HEIC/TIFF はプレビュー描画不可のため対象外）。当面はメインスレッド逐次処理（crop 同様）
...
- 画像編集（ライト/カラー調整 + LUT フィルタ）はプレビュー・出力とも同一の `applyAdjustmentsToCanvas`（`webglImageRenderer.ts`）を通す（WYSIWYG）。既定は WebGL2 フラグメントシェーダ（`adjustmentShader.ts`）でリアルタイム描画し、`createAdjustmentRenderer` が null を返す環境（WebGL2 非対応）は Canvas2D + `applyAdjustmentToPixel`（`adjustments.ts`）+ `applyLutToPixel`（`lutParser.ts`）の CPU パスへフォールバックする。GPU（GLSL）と CPU（TS）が結果一致する構造的担保として、ピクセル単位の色調整数式は `adjustments.ts` の `applyAdjustmentToPixel` を唯一の真実とし、GLSL 側は同じ順序・同じ係数・同じクランプ位置でミラーする（キー→uniform 名の対応表 `ADJUSTMENT_UNIFORMS` も単一の真実として配線・アップロード・単体テストのガードが共有）。バッチ（`editImages`）は WebGL レンダラを 1 個だけ生成して全ファイルで使い回し、finally で必ず dispose する（コンテキスト枯渇を防ぐ。crop 同様メインスレッド逐次処理で、Worker + OffscreenCanvas 化は将来の最適化）。調整は非破壊で `AdjustmentState`（UI 単位 [-100,100]）として保持し、書き出し時に `normalizeAdjustments` で [-1,1] に正規化してから適用する。適用範囲（全画像一括 / 画像ごと）は crop の dual-store パターンを踏襲する（`resolveAdjustmentForIndex`）。UI は `AdjustmentPanel`（`ConversionSettings` の不変更新契約を踏襲）/ `EditToolbar` / `CompareView` / `LutPicker` / `HistogramPanel`（文言は i18n `edit.*` / `edit.lut.*` / `edit.histogram.*`。適用範囲・画像ナビは `crop.*` を共用、失敗通知の見出しは `ConversionErrors` の `titleKey` prop で `edit.editFailures` に差し替え）。ヒストグラム（RGB / 輝度）は調整・LUT 適用後のプレビューを反映する: `CompareView` が編集後キャンバス（GPU / CPU フォールバックのどちらでも転写済みの 2D キャンバス）を `resolveHistogramSampleSize` で縮小 `drawImage` → `getImageData` した ImageData を optional prop `onEditedFrame` で渡し（連続再描画は rAF で 1 フレーム 1 回に間引き・サンプリング用キャンバスは ref で使い回し・コールバックは ref 経由で参照し GPU 再描画を誘発しない）、page 側が `histogram.ts` の `computeHistogram` で 256 ビンの R/G/B/輝度に集計して `HistogramPanel`（SVG パス描画・RGB/輝度チップ）が表示する（「リストをクリア」でリセット）
...
- Canvas API や WASM / WebGL に依存する処理（描画・変換・エンコード）は happy-dom では動作しないため、単体テストの対象外とする（純粋ロジック部分を切り出してテストする）。編集ページも同様で、色調整の数式（`adjustments.ts` の `applyAdjustmentToPixel` 等）・シェーダ生成/配線（`adjustmentShader.ts`）・LUT のパースと CPU 適用（`lutParser.ts` の `parseCubeLut` / `applyLutToPixel` / `haldClutToLutData` 等）・LUT の選択解決（`lutState.ts` の `resolveLutForIndex` 等）・ヒストグラムのビン計算と SVG パス生成（`histogram.ts` の `computeHistogram` / `resolveHistogramSampleSize` / `buildHistogramPath` 等）を純粋ロジックとして単体テスト（`adjustments.test.ts` / `adjustmentShader.test.ts` / `lutParser.test.ts` / `lutState.test.ts` / `histogram.test.ts`）し、WebGL/Canvas を使う実描画（LUT の GPU/CPU 出力ピクセル一致・強度 0 での無効化・プリセット適用・不正ファイル通知、ヒストグラムの輝度スパイク位置・露光量調整への追従・RGB/輝度切替・WebGL2 無効化時の表示を含む）は E2E（`e2e/edit.spec.ts`。`.cube` フィクスチャは `e2e/helpers/fixtures.ts` で実行時生成）で検証する
