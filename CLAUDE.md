...
12. **統合ワークスペース「Image Studio」**（`/studio`）- 6 ツール（トリミング・編集・レタッチ・アップスケール・背景除去・メタデータ）を同一画面で操作する Photoshop ライク。線形パイプライン + 最大 20 コミット undo/redo + 履歴パネル（任意時点復帰・クリア。`editHistory.ts`）。組み込み export で書き出し（リネーム規則トークン対応）
