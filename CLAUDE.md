...
- `src/app/` - Next.js App Router ページ
  ...
  - `crop/` - 画像編集・トリミングツールページ（`components/CropSelector.tsx`（8方向ハンドル操作・比率固定リサイズ）と `components/CropToolbar.tsx`（アスペクト比・回転/反転・色調/フィルタ調整・適用範囲）で構成）
...
- `src/utils/` - コアユーティリティクラス
  ...
  - `imageCropper.ts` - 画像トリミング処理（EXIF Orientation 補正 + 回転/反転 + 色調/フィルタ調整をキャンバスに焼き込む `renderOrientedImage` / `createOrientedPreviewUrl`（`adjustments` 引数対応）、`cropImage`（`transform` / `adjustments` / `quality` 引数対応）、画像ごとの領域・変換・調整を受け取る `cropImages`（`CropJob[]`）。`CropArea` / `CropTransform` は `cropGeometry.ts` から、`ImageAdjustments` は `imageAdjustments.ts` から再エクスポート）
  - `cropGeometry.ts` - トリミングの座標計算に関する Canvas 非依存の純粋ロジック（型 `CropArea` / `CropTransform` / `CropState`（一括/画像ごとの領域・変換・色調調整を保持）、`ASPECT_RATIO_PRESETS`、`clampCropArea` / `scaleCropArea` / `toDisplayArea` / `fitAspectRatio` / `enforceAspectRatio` / `orientedSize` / `rotateRight` / `rotateLeft` / `resolveCropForIndex`（領域・変換に加え色調調整も解決））。CropSelector（表示座標）・crop ページ（自然座標保持）・imageCropper（出力）で共有し単体テスト対象
  - `imageAdjustments.ts` - 色調・フィルタ調整に関する Canvas 非依存の純粋ロジック（型 `ImageAdjustments`（brightness / contrast / saturate / grayscale / sepia）・`AdjustmentSliderKey` / `AdjustmentFilterKey`、定数 `IDENTITY_ADJUSTMENTS` / `ADJUSTMENT_RANGE`、中立でない値のみを CSS filter 文字列へ連結する `buildCanvasFilter`（無ければ "none"）、`isIdentityAdjustments`）。crop ページ（state 保持）・CropToolbar（UI）・imageCropper（`ctx.filter` へ set して焼き込み）で共有し単体テスト対象（cropGeometry.ts と同方針）
...
2. **画像編集・トリミング** - プレビュー付きのビジュアル編集・トリミングインターフェース。アスペクト比プリセット（自由 / 1:1 / 16:9 / 4:3 / 3:2。自由以外は選択中のドラッグ・8方向リサイズ・移動が比率固定）、右/左 90° 回転・左右/上下反転、色調・フィルタ調整（明るさ・コントラスト・彩度のスライダー（0〜200%・100% が中立）+ グレースケール／セピアのワンクリックフィルタ + リセット）、適用範囲の切り替え（「全画像一括」／「画像ごと」。画像ごとは ←/→ ナビゲーションと連動して画像単位に領域・回転/反転・色調調整を保持・適用）に対応。プレビュー時に EXIF Orientation を `createImageBitmap(imageOrientation: "from-image")` で自動補正し、回転/反転・色調調整（Canvas の `ctx.filter`）をキャンバスへ焼き込む（プレビューと出力で同一ロジック＝WYSIWYG。EXIF 保持時は出力の Orientation タグを 1 に正規化して二重回転を防ぐ）。すべての調整は Canvas の `ctx.filter` で実装し WASM 不要・クライアントサイド完結
...
- トリミングの座標計算（境界クランプ・表示⇔自然座標変換・アスペクト比の当てはめ/強制・回転後寸法・画像ごとの領域/変換/色調調整の解決）は Canvas / DOM 非依存の純粋関数 `cropGeometry.ts` に集約し、色調・フィルタ調整の型・CSS filter 文字列の組み立ては同じく純粋関数 `imageAdjustments.ts`（`buildCanvasFilter` など）に集約して、CropSelector（表示座標での操作。`aspectRatio` prop で比率固定リサイズ）・crop ページ（自然座標での state 保持と適用範囲トグル）・`imageCropper.ts`（出力）で共有して単体テストする。回転/反転・色調/フィルタ調整と EXIF Orientation は `renderOrientedImage` が `drawImage` 前の `ctx.filter` 設定と 1 回の描画でプレビューと出力の双方に同一ロジックで焼き込み（WYSIWYG）、EXIF 保持時のみ `normalizeExifForBakedImage` で出力 TIFF の Orientation を 1 に揃え、実ピクセル寸法タグ（PixelXDimension/PixelYDimension）が存在すれば出力寸法へ更新する。色調・フィルタ調整（明るさ・コントラスト・彩度のスライダー + グレースケール／セピア）は Canvas の `ctx.filter` のみで実装し WASM 不要・クライアントサイド完結。トリミングツールバー UI は `src/app/crop/components/CropToolbar.tsx`（アスペクト比・回転/反転・色調/フィルタ調整・適用範囲。文言は i18n `crop.*`）
...
## 最近の更新
- crop ページに色調・フィルタ調整機能を追加（Issue #62）。すべて Canvas の `ctx.filter` で実装し WASM 不要・クライアントサイド完結。明るさ・コントラスト・彩度のスライダー（0〜200%・1% 刻み・100% が中立）、グレースケール／セピアのワンクリックフィルタ、調整を初期値へ戻すリセットボタンを追加。既存の適用範囲トグル（全画像一括 / 画像ごと）に調整も連動し、画像ごとは ←/→ と連動して画像単位に保持する。crop ページの見出し・サブタイトルを「画像編集・トリミング」相当へ拡張。Canvas 非依存の純粋ロジック `imageAdjustments.ts`（型 `ImageAdjustments`（brightness / contrast / saturate / grayscale / sepia）・`AdjustmentSliderKey` / `AdjustmentFilterKey`、定数 `IDENTITY_ADJUSTMENTS` / `ADJUSTMENT_RANGE`、`buildCanvasFilter` / `isIdentityAdjustments`）を新設し単体テスト（`imageAdjustments.test.ts`）。`cropGeometry.ts` の `CropState` に `sharedAdjustments` / `perImageAdjustments`、`resolveCropForIndex` の戻り値に `adjustments` を追加、`imageCropper.ts` の `renderOrientedImage` / `createOrientedPreviewUrl` / `cropImage` に `adjustments` 引数を追加し `drawImage` 前に `ctx.filter = buildCanvasFilter(...)` を設定（プレビューと出力で同一ロジック＝WYSIWYG）・`CropJob` に `adjustments` を追加。UI は `CropToolbar.tsx`（i18n `crop.adjustments` / `crop.brightness` / `crop.contrast` / `crop.saturation` / `crop.grayscale` / `crop.sepia` / `crop.resetAdjustments` を追加、`crop.title` / `crop.subtitle` / `crop.applyModeHelp` を更新）。実ブラウザ検証は `e2e/crop.spec.ts`（グレースケール出力の全画素 R=G=B 検証）。crop / metadata の受理形式・convert の仕様、EXIF 保持・目標サイズ・回転/反転・アスペクト比の既存挙動は不変
