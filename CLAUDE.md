...
- `src/app/` - Next.js App Router ページ
  ...
  - `edit/` - 画像編集ページ（写真アプリ相当のライト/カラー調整 + LUT フィルタ、Issue #66 / #67。`components/AdjustmentPanel.tsx`（ライト/カラーのスライダー群）・`components/EditToolbar.tsx`（適用範囲・出力形式・EXIF 保持・全体リセット）・`components/CompareView.tsx`（分割 Before/After 比較・画像ナビ・WebGL レンダラ保持）・`components/LutPicker.tsx`（LUT プリセットサムネイル一覧・「なし」・カスタムアップロード（.cube / HALD PNG）・適用強度スライダー・エラー通知）で構成）
...
- Canvas API や WASM / WebGL に依存する処理（描画・変換・エンコード）は happy-dom では動作しないため、単体テストの対象外とする（純粋ロジック部分を切り出してテストする）。編集ページも同様で、色調整の数式（`adjustments.ts` の `applyAdjustmentToPixel` 等）・シェーダ生成/配線（`adjustmentShader.ts`）・LUT のパースと CPU 適用（`lutParser.ts` の `parseCubeLut` / `applyLutToPixel` / `haldClutToLutData` 等）・LUT の選択解決（`lutState.ts` の `resolveLutForIndex` 等）を純粋ロジックとして単体テスト（`adjustments.test.ts` / `adjustmentShader.test.ts` / `lutParser.test.ts` / `lutState.test.ts`）し、WebGL/Canvas を使う実描画（LUT の GPU/CPU 出力ピクセル一致・強度 0 での無効化・プリセット適用・不正ファイル通知を含む）は E2E（`e2e/edit.spec.ts`。`.cube` フィクスチャは `e2e/helpers/fixtures.ts` で実行時生成）で検証する
