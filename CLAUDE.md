...
12. **統合ワークスペース「Image Studio」**（`/studio`）- 6 ツール（トリミング・編集・レタッチ・アップスケール・背景除去・メタデータ）を同一画面で操作する Photoshop ライクなワークスペース。線形パイプラインモデル（各ツール出力が次へ入力される）と最大 20 個のコミット単位 undo/redo で複数処理を組み合わせ、組み込み export ダイアログで直接書き出し可。レタッチツールは顔・ナンバープレートの AI 自動検出（UltraFace + LPD-YuNet・ブラウザ内推論）からワンタップでレタッチ領域へ変換できる
