...
- **Integrated Workspace ("Image Studio", `/studio`)**: A unified Photoshop-like workspace where you can use all six editing tools (crop, edit, redact, upscale, background removal, metadata) and export directly, all on the same canvas. No need to switch between separate pages — compose your edits in a linear pipeline where each tool's output feeds into the next. Full undo/redo history with up to 20 commits lets you experiment freely: a history panel (desktop: the "History" button on the tool rail / mobile: a bottom sheet) lists every operation across tools in chronological order, and you can click any row to return to that point, redo forward again, or clear the history back to the original images after confirmation (AI results are cached, so restoring never re-runs inference). Press-and-hold on the canvas temporarily shows the original image from before any edits were applied. The built-in export dialog supports JPEG / PNG / WebP / AVIF with quality and optional resizing control. An optional rename pattern (tokens: `{name}` `{seq}` `{width}` `{height}` `{date}`) lets you control output file names, with a live preview of the actual names. The redact tool includes AI auto-detection of faces and license plates (UltraFace + LPD-YuNet, ~6MB total, running entirely in the browser with ONNX Runtime): detected candidates are highlighted on the canvas with dashed outlines and category labels, and one tap converts the selected categories into regular redaction regions (with a small padding margin) that you can move, resize, or delete just like manually drawn ones. The models are downloaded on first use and cached locally; your images are never uploaded.
