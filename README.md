...
- **Image Editing (Light / Color adjustments)**: Photo-app-style adjustments — Light (exposure, brightness, contrast, highlights, shadows, whites, blacks) and Color (saturation, vibrance, temperature, tint, hue) — applied non-destructively with a real-time before/after preview and applied to multiple images at once. Rendering uses a WebGL2 shader with an automatic Canvas2D CPU fallback (the preview and the output share the same render path, so they match). Output keeps the original format or converts to JPEG / PNG / WebP / AVIF.
