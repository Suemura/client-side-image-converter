...
- **Image Editing (Light / Color adjustments + LUT filters)**: Photo-app-style adjustments — Light (exposure, brightness, contrast, highlights, shadows, whites, blacks) and Color (saturation, vibrance, temperature, tint, hue) — applied non-destructively with a real-time before/after preview and applied to multiple images at once. You can also apply LUT (Look-Up Table) color filters: pick from 11 bundled presets or upload your own `.cube` (1D / 3D) or HALD CLUT PNG, and control the blend strength. A live histogram (switchable between RGB and luminance) reflects the adjusted result in real time as a guide for black/white level tuning. Rendering uses a WebGL2 shader with an automatic Canvas2D CPU fallback (the preview and the output share the same render path, so they match). Output keeps the original format or converts to JPEG / PNG / WebP / AVIF.
