...
- **Tool Chaining (send results to the next tool)**: Hand off conversion/optimization or cropping results directly to another tool and keep processing without downloading them first (currently between Convert and Crop; e.g. convert PNG to JPEG, then crop). The handoff happens entirely in browser memory, and the send buttons only appear for tools that accept the resulting formats.
