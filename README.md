...
- **EXIF Metadata Management**: View EXIF data (JPEG / PNG / WebP), edit tags, and selectively remove sensitive metadata. GPS location can be removed or rounded to roughly city level (about 1 km precision, JPEG only). Content Provenance (C2PA) is also supported: view and verify content credentials and AI-generation signatures, and remove them to protect privacy (all client-side, no server transmission).
