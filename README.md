...
- **Image Editing & Cropping**: Visual editing tool with live preview, aspect-ratio presets (Free / 1:1 / 16:9 / 4:3 / 3:2), 90° rotation and horizontal/vertical flip with automatic EXIF Orientation correction, color and filter adjustments (brightness / contrast / saturation sliders plus one-click grayscale / sepia filters, all done with the Canvas filter API — no WASM), and a choice between applying one setting to all images or editing each image individually.
