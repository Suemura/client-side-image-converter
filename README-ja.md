...
- **ツール連携（結果を次のツールへ）**: 変換・最適化やトリミング、画像編集、メタデータ削除の結果を、ダウンロードせずにそのまま別のツールへ引き継いで続けて処理できます（変換・トリミング・編集・メタデータ編集の全 4 ツールが相互に対応。例: 画像を編集 → JPEG に変換 → トリミング → メタデータを削除）。メタデータページではタグを選択すると「選択したメタデータを削除して次のツールへ」の導線が表示され、削除と引き継ぎを一度に行えます。引き継ぎはブラウザのメモリ上だけで行われ、結果の形式を受け付けられるツールにのみ引き継ぎボタンが表示されます。
