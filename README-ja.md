...
- **ツール連携（結果を次のツールへ）**: 変換・最適化やトリミングの結果を、ダウンロードせずにそのまま別のツールへ引き継いで続けて処理できます（現在は変換 ⇄ トリミングに対応。例: PNG を JPEG に変換してからトリミング）。引き継ぎはブラウザのメモリ上だけで行われ、結果の形式を受け付けられるツールにのみ引き継ぎボタンが表示されます。
