...
- **画像編集（ライト/カラー調整 + LUT フィルタ）**: 写真アプリ相当の調整 —— ライト（露光量・輝度・コントラスト・ハイライト・シャドウ・白/黒レベル）とカラー（彩度・自然な彩度・色温度・色合い・色相）—— を、Before/After のリアルタイムプレビューを見ながら非破壊で行い、複数画像へまとめて適用できます。さらに LUT（Look-Up Table）による色変換フィルタも適用でき、同梱プリセット（11 種）から選ぶか、`.cube`（1D / 3D）や HALD CLUT PNG を自分でアップロードして使え、適用強度でブレンド比を調整できます。調整・LUT 適用後の結果を反映するヒストグラム（RGB / 輝度を切替可能）もリアルタイムに表示され、黒/白レベル調整の目安になります。描画は WebGL2 シェーダを使い、非対応環境では Canvas2D の CPU パスへ自動フォールバックします（プレビューと出力は同じ描画経路を通るため結果が一致）。出力は元形式維持または JPEG / PNG / WebP / AVIF への変換に対応。
