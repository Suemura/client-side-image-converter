...
- **画像編集・トリミング**: プレビュー付きのビジュアル編集・トリミングツール。アスペクト比プリセット（自由 / 1:1 / 16:9 / 4:3 / 3:2）、90° 回転・左右/上下反転（EXIF Orientation の自動補正付き）、色調・フィルタ調整（明るさ・コントラスト・彩度のスライダーとグレースケール／セピアのワンクリックフィルタ。すべて Canvas の filter API で実装し WASM 不要）、「全画像一括」／「画像ごと」に領域・回転・色調を適用できる切り替えに対応。
