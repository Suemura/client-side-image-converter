...
- **EXIF メタデータ管理**: EXIF 情報の表示（JPEG / PNG / WebP に対応）、タグの編集、機微なメタデータの選択的削除。GPS 位置情報は削除に加えて、市区町村レベル（約 1km 精度）に丸めて残すことも選べます（JPEG のみ）。また、コンテンツ来歴（C2PA / Content Credentials）にも対応しており、コンテンツ認証情報と AI 生成署名を表示・検証でき、プライバシー保護のため除去することも可能です（すべてブラウザ内で完結）。
