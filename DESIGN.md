...
| トークン | ライト | ダーク | 用途 |
| --- | --- | --- | --- |
| `--canvas-bg` | `#e9e9ec` | `#000000` | キャンバス領域の背景（画像の視認用に一段暗い中立色。テキストは置かない） |
| `--rail-bg` | `#ffffff` | `#141414` | ツールレール・フィルムストリップの背景 |

`--canvas-bg` は画像プレビューの台紙専用で、テキスト・アイコンの背景に使わない
（コントラスト基準の対象外とするため）。他ページへの流用は禁止。
