...
- 非推奨エイリアス `--primary` / `--border-light` / `--border-dashed` が残存
  （`src/components/` の参照は Phase 2 = Issue #79、`/crop` + `/metadata` は Phase 4 = Issue #81、
  `src/app/edit/` は Phase 5 = Issue #82 で解消済み。残るトップ / `/convert` の参照を
  Phase 3 = Issue #80 で新トークンへ置換し、`globals.css` からのエイリアス定義削除は
  全フェーズのマージ後のフォローアップで行う）
- module.css のハードコード色・インラインスタイルの解消は `src/components/`
  （Phase 2 = Issue #79。`globals.css` のユーティリティクラスは全廃済み）・
  `/crop` + `/metadata` の **module.css**（Phase 4 = Issue #81）・`/edit`（Phase 5 = Issue #82）
  まで完了（画像上オーバーレイ UI・データ可視化色の固定色は例外規定に従いコメント付きで維持）。
  残るトップ / `/convert`（Phase 3 = Issue #80）で解消予定
