...
- 非推奨エイリアス `--primary` / `--border-light` / `--border-dashed` が残存
  （`src/components/` の参照は Phase 2 = Issue #79、`src/app/edit/` の参照は Phase 5 = Issue #82 で
  解消済み。残るページ側（トップ / /convert / /crop / /metadata）の参照は Phase 3 = Issue #80・
  Phase 4 = Issue #81 で新トークンへ置換し、`globals.css` からのエイリアス定義削除は
  両フェーズのマージ後のフォローアップで行う）
- module.css のハードコード色・インラインスタイルの解消は `src/components/`
  （Phase 2 = Issue #79。`globals.css` のユーティリティクラスは全廃済み）と `/edit`
  （Phase 5 = Issue #82。画像上 UI・データ可視化色としてコメント付きで残す固定色を除く）まで完了。
  残るページ（トップ / /convert / /crop / /metadata）は Phase 3・4 で解消予定
