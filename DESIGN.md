...
- 本文テキストは背景に対して **4.5:1 以上**、大きめ文字（18px 以上または 14px 太字以上）と UI 部品の輪郭は **3:1 以上**
...
- キーボードフォーカスは `:focus-visible` 相当（hidden input のため `:focus-within` で代替可）
...
- タイトル 16px / 500 `--foreground`、説明 14px `--muted-foreground`、ヒント 12px `--faint-foreground`
...
| `max-width: 768px` | 標準のモバイル切替（フッター段組解除・モーダル余白縮小等） |
| `max-width: 640px` | ヘッダーの文字ラベル省略等、最終段の圧縮 |
...
- 非推奨エイリアス `--primary` / `--border-light` / `--border-dashed` が残存
  （Phase 2〜5 で参照元を新トークンへ置換し、Phase 5 で削除予定）
- module.css のハードコード色・ユーティリティクラス・インラインスタイルは Phase 2〜5 で段階的に解消予定
  （本書策定時点では旧デザインのままのコンポーネントが残る）
