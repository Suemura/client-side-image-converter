...
- 非推奨エイリアス `--primary` / `--border-light` / `--border-dashed` が残存
  （`src/components/` は Phase 2 = Issue #79、トップ `/` + `/convert` は Phase 3 = Issue #80 で
  参照解消済み。残る `/crop` + `/metadata` は Phase 4 = Issue #81、`/edit` は Phase 5 = Issue #82 で
  新トークンへ置換し、Phase 5 でエイリアス自体を削除予定）
- module.css のハードコード色・インラインスタイルの解消は `src/components/`
  （Phase 2 = Issue #79。`globals.css` のユーティリティクラスは全廃済み）と
  トップ `/` + `/convert`（Phase 3 = Issue #80）まで完了。
  残る `/crop` + `/metadata` は Phase 4、`/edit` は Phase 5 で解消予定
