...
- 非推奨エイリアス `--primary` / `--border-light` / `--border-dashed` が残存
  （`src/components/` の参照は Phase 2 = Issue #79 で解消済み。残るページ側の参照を
  Phase 3〜5 で新トークンへ置換し、Phase 5 で削除予定）
- module.css のハードコード色・インラインスタイルの解消は `src/components/` まで完了
  （Phase 2 = Issue #79。`globals.css` のユーティリティクラスは全廃済み）。
  ページ側（`src/app/` 配下）は Phase 3〜5 で段階的に解消予定
