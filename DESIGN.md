...
- 非推奨エイリアス `--primary` / `--border-light` / `--border-dashed` が残存
  （`src/components/` の参照は Phase 2 = Issue #79、`/crop` + `/metadata` は Phase 4 = Issue #81 で
  解消済み。残るトップ / `/convert` / `/edit` の参照を Phase 3 = Issue #80・Phase 5 = Issue #82 で
  新トークンへ置換し、Phase 5 で削除予定）
- module.css のハードコード色・インラインスタイルの解消は `src/components/`
  （Phase 2 = Issue #79。`globals.css` のユーティリティクラスは全廃済み）と
  `/crop` + `/metadata` の **module.css**（Phase 4 = Issue #81。画像上オーバーレイ UI の固定色は
  例外規定に従いコメント付きで維持）まで完了。トップ / `/convert`（Phase 3 = Issue #80）と
  `/edit`（Phase 5 = Issue #82）で残りを解消予定
